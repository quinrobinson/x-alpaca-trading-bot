...
| 2026-08-04 | Dashboard rework | Post-X dashboard rework (branch dashboard-post-x-rework). /timeline is now a three-source merge (X archive + scanner_events + scanner_trades) — every item carries `ts` (merge key) + globally-unique `key` (x-/se-/st- prefix); new kinds scanner_event / scanner_trade_open / scanner_trade_closed with matching Timeline.jsx cards. Dashboard home = scanner "right now": open S2 shorts lead (entry/stop/60-min note), legacy option cards below, empty-state copy reflects armed/disarmed. Performance gets "Retired strategy · historical record" banner (45-trade baseline kept). Settings: daily-loss hint notes both books share the kill switch; spend/qty/pause demoted under a "Legacy · X options (retired)" header. test_api conn fixture now also cleans scanner tables (multi-source timeline leaked rows across test modules). 460 Python tests + build + vitest green. |
